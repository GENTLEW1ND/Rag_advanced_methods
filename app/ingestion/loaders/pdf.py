...
def parse_pdf(file_path: str) -> str:
    ...
    Falls back to pdfplumber for pages that yields no text (e.g. image-heave pages).    
    """
    with logfire.span("PDF parsing (local)", filename=file_path):
        try:
            ...
            # Fallback: use pdfplumber for any pages pypdf returned black
            if blank_pages:
            ...
            if not full_text.strip():
                logfire.warning(f"No text extracted from {file_path}. File may be fully image_based.")
            else:
