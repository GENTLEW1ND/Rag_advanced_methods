...
from app.ingestion.services.retrieval.embeddings import embed_text, get_embedding_dim
...
def process_directory(dir_path: str, filename: str, source_type: str):
...
def run_universal_ingestion(base_dir: str, explicit_source_type: str = None, wipe: bool = False):
    ...
    """
    with logfire.span("Universe Ingestion Started", base_directory = base_dir):
        # Recreate collection - dimesion resolved at runtime after embedding model probe
        if not qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            ...
            logfire.info(
                f"Create collection '(settings.QDRANT_COLLECTION)'"
                f"({dim} - dim, Cosine)"
            )
