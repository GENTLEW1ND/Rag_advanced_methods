...
def _probe_gemini():
    ...
    try:
        ...
        logfire.warning(f"Gemini probe failed: {e}. Will use sentence-trasformers fallback.")
        return None
...
def get_embedding_dim() -> int:
    """Retrun the vector dimension for the active model. Call after _init()."""
    _init()
...
def embed_text(texts: list[str]) -> list[list[float]]:
    ...
    for i in range(0, len(texts), BATCHA_SIZE):
